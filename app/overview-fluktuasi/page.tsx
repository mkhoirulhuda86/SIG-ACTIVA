...
const fmtCompact = (value: number) => {
  ...
  const sign = value < 0 ? '-' : '';
  if (abs >= 1_000_000_000) return `${sign}${(abs / 1_000_000_000).toLocaleString('id-ID', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} M`;
  if (abs >= 1_000_000) return `${sign}${(abs / 1_000_000).toLocaleString('id-ID', { minimumFractionDigits: 1, maximumFractionDigits: 1 })} Jt`;
  if (abs >= 1_000) return `${sign}${(abs / 1_000).toLocaleString('id-ID', { maximumFractionDigits: 1 })} Rb`;
...
export default function OverviewFluktuasiPage() {
  ...
  const loadDashboard = useCallback(async (periode?: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/fluktuasi/dashboard${query}`, { cache: 'no-store' });
    ...
    <div className="flex h-dvh overflow-hidden bg-[#edf2f6]">
      ...
      <div className="flex min-w-0 flex-1 flex-col lg:ml-64">
        <Header
          ...
          subtitle="MoM, YoY, YTD dan Key Reasons dari data Fluktuasi OI/EXP yang sama"
                              ...
                              <>
                                <div className="mt-5 grid grid-cols-[1fr_auto] items-end gap-3">
                                  ...
                                  <div className="text-right">
                                    ...
                                    <div className="mt-1 text-base font-black text-[#17324a]">{fmtCompact(data.kpi.movement)}</div>
                                  </div>
                                </div>
                                ...
                                <div className="mt-4 grid grid-cols-2 gap-2">
                                  <div className="rounded-[13px] border border-slate-200/80 bg-gradient-to-b from-white/95 to-[#f7fafc] p-2.5 shadow-[0_4px_10px_rgba(29,57,83,0.04)]">
                                    ...
                                    <b className="mt-1 block text-[11px] text-[#17324a]">{fmtCompact(data.kpi.current)}</b>
                                  </div>
                                  <div className="rounded-[13px] border border-slate-200/80 bg-gradient-to-b from-white/95 to-[#f7fafc] p-2.5 shadow-[0_4px_10px_rgba(29,57,83,0.04)]">
                                    <span className="block text-[7px] font-extrabold uppercase text-[#8192a2]">{meta.previous}</span>
                                    <b className="mt-1 block text-[11px] text-[#17324a]">{fmtCompact(data.kpi.previous)}</b>
                                  </div>
                                </div>

                                <div className="mt-4 border-t border-slate-300/50 pt-3">
                                  <div className="flex items-center justify-between gap-2 text-[8px] font-bold uppercase tracking-[0.06em] text-[#748596]">
                                    <span>Key Reasons · Pareto 80%</span>
                                    <span>{data.pareto.length} driver</span>
                                  </div>
                                  <div className="mt-2 space-y-2">
                                    {data.pareto.length === 0 ? (
                                      <div className="rounded-xl border border-slate-200/70 bg-white/45 px-3 py-2 text-[9px] text-slate-400">Tidak ada movement.</div>
                                    ) : data.pareto.slice(0, 4).map((driver) => (
                                      <div key={driver.accountCode} className="rounded-xl border border-slate-200/70 bg-white/55 px-3 py-2">
                                        <div className="flex items-start justify-between gap-3">
                                          <div className="min-w-0">
                                            <div className="text-[9px] font-black text-[#2c4b62]">{driver.accountCode}</div>
                                            <div className="mt-0.5 truncate text-[8px] text-[#617588]">{driver.description}</div>
                                          </div>
                                          <div className="shrink-0 text-right text-[8px] font-bold text-slate-600">
                                            {(driver.paretoContribution * 100).toLocaleString('id-ID', { maximumFractionDigits: 1 })}%
                                          </div>
                                        </div>
                                        <div className="mt-1.5 whitespace-pre-wrap text-[8px] leading-4 text-[#42596e]">
                                          {driver.reason || <span className="text-slate-400">Belum ada narasi Reason.</span>}
                                        </div>
                                      </div>
                                    ))}
                                  </div>
