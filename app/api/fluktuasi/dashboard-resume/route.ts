...
import {
  ...
  previousMonth,
  previousYear,
  selectBestSnapshot as chooseBestSnapshot,
  snapshotReasonMatches,
  ytdPeriods,
} from '@/lib/fluktuasi/dashboard-resume';
...
function buildSnapshotMeta(snapshot: SnapshotCandidate | null) {
  ...
  if (!snapshot) {
    for (const item of CLASSIFICATIONS) accountOrder.set(item.key, [...item.accounts]);
  ...
  for (const item of CLASSIFICATIONS) {
    const allowed = new Set(item.accounts);
    const ordered = (accountOrder.get(item.key) ?? []).filter((code) => allowed.has(code));
    for (const code of item.accounts) if (!ordered.includes(code)) ordered.push(code);
...
function getSnapshotReason(row: RekapRow | undefined, activity: ActivityKey): string {
  if (!row) return '';
  ...
  return String(row[sourceField] ?? '').trim() || String(row[legacyField] ?? '').trim();
}
...
function activityPeriods(activity: ActivityKey, currentPeriod: string) {
  ...
  return {
    ...
    currentLabel,
    previousLabel,
    comparisonPeriod: previousPeriods.at(-1) ?? previousYear(currentPeriod),
    comparisonType: 'YTD' as const,
  ...
  try {
    ...
    if (requestedPeriod && !/^\d{4}\.\d{2}$/.test(requestedPeriod)) {
      return NextResponse.json(
        { success: false, error: 'Format periode harus YYYY.MM' },
        { status: 400, headers: NO_STORE_HEADERS },
      );
    }

    const [amountRecords, imports, finalReasons] = await Promise.all([
    ...
    const periodOptions = [...new Set(
      amountRecords.map((row) => row.periode).filter((p) => /^\d{4}\.\d{2}$/.test(p)),
    )].sort();
    const period = requestedPeriod || periodOptions.at(-1) || '';
    if (!period) {
      return NextResponse.json(
        { success: true, data: { period: '', periodLabel: '', periodOptions: [], classifications: [], source: null } },
        { headers: NO_STORE_HEADERS },
      );
    }

    const snapshot = selectBestSnapshot(imports, period);
    const { accountOrder, descriptions, reasons } = buildSnapshotMeta(snapshot);
    ...
    const finalReasonMap = new Map<string, FinalReasonRow>();
    for (const row of finalReasons) {
      finalReasonMap.set(
        `${row.accountCode}|${row.comparisonType}|${row.currentPeriod}|${row.comparisonPeriod}`,
        row,
      );
    }

    const classifications = CLASSIFICATIONS.map((classification) => {
      const accounts = accountOrder.get(classification.key) ?? classification.accounts;
      const activities = Object.fromEntries((['mom', 'yoy', 'ytd'] as ActivityKey[]).map((activity) => {
        const periods = activityPeriods(activity, period);
        const missingPeriods = [...periods.currentPeriods, ...periods.previousPeriods]
          .filter((p) => !availablePeriods.has(p));
        ...
        const rawRows = accounts.map((accountCode) => {
          ...
          const override = finalReasonMap.get(
            `${accountCode}|${periods.comparisonType}|${period}|${periods.comparisonPeriod}`,
          );
          const reviewedReason = String(override?.userComment ?? '').trim()
            || String(override?.generatedReason ?? '').trim();

          const snapshotRow = reasons.get(accountCode);
          const snapshotMovement = activity === 'mom'
            ? snapshotRow?.gapMoM
            : activity === 'yoy'
              ? snapshotRow?.gapYoY
              : snapshotRow?.gapYtD;
          const snapshotReason = snapshotReasonMatches(snapshotMovement, movement)
            ? getSnapshotReason(snapshotRow, activity)
            : '';

          const fallbackRecords = periods.currentPeriods
            .map((p) => recordMap.get(`${accountCode}|${p}`))
            .filter(Boolean);
          const fallbackReason = [...new Set(
            fallbackRecords
              .map((record) => String(record?.remark || record?.klasifikasi || '').trim())
              .filter(Boolean),
          )].join('; ');
          ...
          return {
            ...
            reason: reviewedReason || snapshotReason || fallbackReason,
          };
        });

        const rankedRows = buildPareto(rawRows);
        const rankMap = new Map(rankedRows.map((row) => [row.accountCode, row]));
        const rows = rawRows.map((row) => rankMap.get(row.accountCode) ?? {
          ...row,
          paretoContribution: 0,
          paretoCumulative: 0,
          paretoSelected: false,
        });
    ...
    return NextResponse.json(
      { success: false, error: 'Gagal membangun Dashboard Resume fluktuasi' },
